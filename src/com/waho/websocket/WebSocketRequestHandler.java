		...
		if (msg.getCmd().equals("heartbeat")) {
			...
			if (msg.getMac() != null) {
				...
				try {
					...
					//Node selectResult = nodeDao.selectNodeByMac(node.getMac()); //获取节点数据库内信息
					/**
					 * 若心跳包上传的状态与数据库存储不同，
					 * 进行更新节点属性，且添加节点状态记录到数据库
					 * 注意：若节点后续添加了其他的属性，则此处需要添加新的判断条件
					 */
					if (selectResult.getMac().equals(node.getMac()) == false
							|| selectResult.getType() != node.getType()
							|| selectResult.getPower() != node.getPower()
							|| selectResult.getSsid().equals(node.getSsid()) == false
							|| selectResult.getPw().equals(node.getPw()) == false
