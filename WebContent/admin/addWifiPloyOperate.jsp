...
<body>
				...
				<div class="selectFunction">
					...
					<div class="selectFunctionDiv">
						<select id="selected" lay-verify="" lay-filter="selected">
						  <option value="wifidim" class="i18n" name="LDim"></option>
						  <option value="autoDim" class="i18n" name="AutoDim"></option>
						</select> 
					</div>
				</div>
				<div class="paramDiv">
					<div class="dimValue" id="dimDiv" style="display:block;">
					...
					<div class="dimValue" id="autoDimDiv" style="display:none;">
						<label class="i18n" name="EnterDimPara"></label>
						<div style="margin-top:10px;margin-left:40px;width:80px;">
							<input type="text" id="autoDimInput" placeholder="1-60000" 
								autocomplete="off" class="layui-input" onchange="checkAutoDim(this)">  
						</div>
					</div>
				</div>
			</div>
	...
	*/
	layui.use(['element','slider','form','layer'], function(){
		  ...
		  form.on('select(selected)', function(data){
			  ...
			  var autoDim = document.getElementById("autoDimDiv");
			  switch(data.value){
		         case "autoDim":
		           //切换至自动调光功能
		            dim.style.display = "none";
		            autoDim.style.display = "block";
			        break;
			     case "wifidim":
		            //切换至调光功能
		            dim.style.display = "block";
		            autoDim.style.display = "none";
		            break;  
		        }
			});    
	});    
  ...
  //3.控制PWM调光范围
	function checkDim(obj){
	 ...
	 //3.设置自动调光范围
	function checkAutoDim(obj){
		var val = document.getElementById("autoDimInput").value;
		//1-60000的正则表达式
		var reg = new RegExp("^(?:[1-9]{1,5}|60000)$");
		if(!reg.test(val)){
			layui.use(['layer'], function(){
				  var layer = layui.layer;
				  if(val > 60000){
				  ...
				  layer.msg(jQuery.i18n.prop('60000Int'));
				  //  layer.msg("请输入1-60000以内的整数");
			});
		}else{
		      ...
		      var regDate = /^[12]\d{3}\/(?:0[1-9]|1[0-2])\/(?:0[1-9]|[12][0-9]|30|31)$/;
	 		  ...
	 		  if(hours > 23 || hours < 0 || hours == ""){//判断时间格式
	 			  ...
	 			  switch(selected){
	 			     ...
	 			     case "autoDim":
		 		        //切换至自动调光功能，value为1-60000的整数
		 		        functionStr = "autoDiming";
		 		        value = document.getElementById("autoDimInput").value;
		 		        if(value == ""){
		 		            layer.msg(jQuery.i18n.prop('DimParaNULL'));//未输入调光值!
		 		         	return;
		 		         }else{
