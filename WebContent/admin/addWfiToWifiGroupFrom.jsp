...
<body>
	<form class="layui-form">
		...
		<div class="layui-form-item" style="margin-left:15px;margin-top:15px;">
			...
			<c:set var="length" scope="session" value="${wifiListSize }" />
			<!-- 无节点，提示 -->
	    	<c:if test="${length == 0}">
		    ...
		    <c:if test="${length != 0}">
				<div class="layui-input-block" style="margin-left:100px;">
					...
					<c:forEach items="${wifis}" var="wifi">
						<input type="checkbox" name="checkOne" value="${wifi.mac}" class="check" lay-skin="primary" title="${wifi.nodeName}"></br>
					</c:forEach>
				</div>
			</c:if>
		</div>
		<div class="layui-form-item">
			<div class="layui-input-block">
				<a class="layui-btn layui-btn-sm" 
				  onclick="submitBtn('${pageContext.request.contextPath }/addWifToGroupServlet',${groupid})">
